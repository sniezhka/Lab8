...
#include"Human.h"
class Order;
class Admin :virtual public Human {//1 віртуальне наслідування
	string work;
	 int salary;//12
	 int wallet;
	 int premium;
public:
	...
	void who_is_this_man() { cout << "Адмін" << endl; };//реалізація 2
	int cardnumber;
	Admin(string, string, string, int, int, int, int, string);//конструктор  4
	...
	void SetName(string);
	void SetName();
	void AddToBlackList(Order);//додання замовлення до чорного списку
